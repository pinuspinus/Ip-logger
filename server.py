...
async def redirect_link(request: Request, slug: str):
    ...
    try:
        ...
        # Атомарный инкремент: только если clicks < max_clicks
        cur.execute(
            "UPDATE links SET clicks = clicks + 1 WHERE id = ? AND clicks < max_clicks",
            (link_id,)
        )
        conn.commit()

        if cur.rowcount == 0:
            # Лимит уже исчерпан — НЕ редиректим и уведомляем владельца
                    ...
                    async with httpx.AsyncClient() as client:
                        await client.post(
                            ...
                            data={
                                "chat_id": chat_id,
                                "text": (
                                    "<b>⚠️ Попытка перехода, но лимит исчерпан!</b>\n\n"
                                    "⏳ Ссылка больше недоступна ⏳\n"
                                    "❌ Пользователь не перенаправлен ❌\n\n"
